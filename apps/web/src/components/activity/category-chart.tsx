...
import { categoryLabels, mockAlerts } from "./activity-data";
import type { AlertCategory } from "./activity-data";
...
const categoryData = categories.map((category, index) => ({
  ...
  count: mockAlerts.filter((alert) => alert.category === category).length,
}));

const chartConfig: ChartConfig = Object.fromEntries(
  categoryData.map((entry) => [
    entry.category,
    { color: entry.color, label: categoryLabels[entry.category] },
  ])
);
