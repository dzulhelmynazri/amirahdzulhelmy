...
export default defineChannel({
  events: {
    async "input.requested"(event, channel, ctx) {
      ...
      `;
      await sendEmail({
        idempotencyKey: `disruption-guard-input/${ctx.session.id}/${request.requestId}`,
        subject:
          request.kind === "tool-approval"
            ? "Approval needed — Disruption Guard"
            : "Response needed — Disruption Guard",
        text: `${request.prompt}\n\n${footer}`,
        to,
      });
    },
    ...
    async "message.completed"(event, channel, ctx) {
      ...
      await sendEmail({
        idempotencyKey: `disruption-guard-reply/${ctx.session.id}/${event.sequence}`,
        subject: "Disruption Guard — flight update",
        text: event.message,
        to,
      });
    },
