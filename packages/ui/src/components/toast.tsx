...
const defaultActionRender = <Button variant="outline" size="sm" />;

const ToastAction = ({
  className,
  render = defaultActionRender,
  ...props
...
const defaultCloseRender = <Button variant="ghost" size="icon-sm" />;

const ToastClose = ({
  className,
  children,
  render = defaultCloseRender,
  ...props
...
const ToastIcon = ({ type }: { type: string | undefined }) => {
  ...
  if (type === "info") {
    icon = <Info strokeWidth={2} aria-hidden="true" />;
  }

  if (type === "warning") {
    icon = <TriangleAlert strokeWidth={2} aria-hidden="true" />;
  }
